...
class Metadata:
    ...
    def __init__(self,
        ...
        try:
            self.validation_value = list(set([
                val for val in self.metadata[self.reference_column]
    ...
    def _lookup(self,
                file_name: str,
                which: Literal["batch", "reference_file", "reference_value"]) -> str:
        if which == "batch":
        ...
        return self.metadata.loc[
            self.metadata[self.sample_identifier_column] == file_name,
            lookup_col
        ].iloc[0]
    ...
    def get_corresponding_reference_file(self,
                                         file_name) -> str:
    ...
    def add_file_to_metadata(self,
                             file_name: str,
                             batch: Union[str, int]) -> None:
    ...
    def convert_batch_dtype(self) -> None:
        ...
        if not is_numeric_dtype(self.metadata[self.batch_column]):
            try:
                self.metadata[self.batch_column] = \
                    self.metadata[self.batch_column].astype(np.int8)
            except ValueError:
                self.metadata[f"original_{self.batch_column}"] = \
                    self.metadata[self.batch_column]
                mapping = {entry: i for i, entry in enumerate(self.metadata[self.batch_column].unique())}
                self.metadata[self.batch_column] = \
                    self.metadata[self.batch_column].map(mapping)

    def validate_metadata_table(self):
        if not all(k in self.metadata.columns
                   for k in [self.sample_identifier_column,
                             self.reference_column,
                             self.batch_column]):
            raise ValueError(
        ...
        if not _conclusive_reference_values(self.metadata,
                                            self.reference_column):
            raise ValueError(
    ...
    def assemble_reference_assembly_dict(self):
        ...
        self.reference_assembly_dict = {
            batch: self.get_files_per_batch(batch)

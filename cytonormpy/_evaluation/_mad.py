...
def mad_from_anndata(
    ...
    df = _calculate_mads_per_frame(df, channels, groupby)
...
def mad_comparison_from_fcs(
    ...
    orig_df = mad_from_fcs(origin="original", files=orig_files, **kwargs)
    norm_df = mad_from_fcs(origin="normalized", files=norm_files, **kwargs)
    ...
    if "file_name" in df.index.names:
        ...
        df["file_name"] = [entry.strip(norm_prefix + "_") for entry in df["file_name"].tolist()]

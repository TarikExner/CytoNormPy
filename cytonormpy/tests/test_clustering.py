...
def test_run_clustering_with_markers(data_anndata: AnnData,
    ...
    cn.run_clustering(n_cells = 100,
                      test_cluster_cv = True,
                      cluster_cv_threshold = 2,
                      markers = detector_subset)
    assert "clusters" in cn._datahandler.ref_data_df.index.names
    assert cn._datahandler.ref_data_df.shape == original_shape

def test_wrong_input_shape_for_clustering(data_anndata: AnnData,
                                          detector_subset: list[str]):
...
def test_wrong_input_shape_for_clustering_kmeans(data_anndata: AnnData,
                                                 detector_subset: list[str]):
    cn = CytoNorm()
    cn.run_anndata_setup(adata = data_anndata)
    cn.add_transformer(AsinhTransformer())

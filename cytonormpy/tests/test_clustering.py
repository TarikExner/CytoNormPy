...
from cytonormpy._clustering._cluster_algorithms import FlowSOM, ClusterBase, KMeans, AffinityPropagation, MeanShift
...
def make_indexed_df(sample_ids: list[str], n_cells: int) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        data=np.zeros((n_cells, 1)), 
        index=pd.Index(idx, name="file"),
        columns=["dummy"]
    )
...
def test_calculate_cluster_cvs_structure(monkeypatch):
    ...
    assign = np.vstack([
        np.zeros(6, int),
        np.concatenate([np.zeros(3,int), np.ones(3,int)])
    ]).T  # shape (6,2)
...
def test_kmeans_multiple_varies_clusters(toy_data):
    ...
    # shape correct
    assert out.shape == (6, 3)
    diffs = [not np.array_equal(out[:, i], out[:, j]) 

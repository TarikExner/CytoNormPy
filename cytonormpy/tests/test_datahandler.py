...
def test_correct_channel_indices_markers_fcs(metadata: pd.DataFrame,
                                             INPUT_DIR: Path):
    dh = DataHandlerFCS(
        metadata=metadata,
        input_directory=INPUT_DIR,
...
def test_correct_channel_indices_list_fcs(metadata: pd.DataFrame,
                                          detectors: list[str],
                                          INPUT_DIR: Path):
    subset = detectors[:30]
    dh = DataHandlerFCS(
...
def test_correct_channel_indices_list_anndata(data_anndata: AnnData,
                                              detectors: list[str],
                                              DATAHANDLER_DEFAULT_KWARGS: dict):
    subset = detectors[:30]
    kwargs = DATAHANDLER_DEFAULT_KWARGS.copy()
...
def test_get_batch_anndata(datahandleranndata: DataHandlerAnnData,
...
def test_find_corresponding_reference_file_anndata(datahandleranndata: DataHandlerAnnData,
                                                   metadata: pd.DataFrame):
    dh = datahandleranndata
...
def test_get_corresponding_ref_dataframe(datahandleranndata: DataHandlerAnnData,
    ...
    assert not np.allclose(
        ref_df.iloc[:14].values,
        sample_df.iloc[:14].values
    )
...
def test_artificial_ref_on_relabeled_batch_anndata(data_anndata: AnnData,
                                                   DATAHANDLER_DEFAULT_KWARGS: dict):
...
def test_artificial_ref_on_relabeled_batch_fcs(metadata: pd.DataFrame,
                                              INPUT_DIR: str):
    # relabel so chosen batch has no true reference samples
    md = metadata.copy()
...
def test_add_file_fcs_updates_metadata_and_provider(metadata: pd.DataFrame,
                                                    INPUT_DIR: Path,
                                                    DATAHANDLER_DEFAULT_KWARGS: dict):
    dh = DataHandlerFCS(
...
def test_string_batch_conversion_fcs(metadata: pd.DataFrame,
                                     INPUT_DIR: Path,
                                     DATAHANDLER_DEFAULT_KWARGS: dict):
    md = metadata.copy()
...
def test_string_batch_conversion_anndata(data_anndata: AnnData,
                                         DATAHANDLER_DEFAULT_KWARGS: dict):
    ad = data_anndata.copy()
...
def test_marker_selection_filters_columns(datahandleranndata: DataHandlerAnnData,
                                          detectors: list[str],
...
def test_marker_selection_subsampled_filters_and_counts(datahandleranndata: DataHandlerAnnData,
                                                        detectors: list[str],
                                                        detector_subset: list[str],
                                                        DATAHANDLER_DEFAULT_KWARGS: dict):
    dh = datahandleranndata
